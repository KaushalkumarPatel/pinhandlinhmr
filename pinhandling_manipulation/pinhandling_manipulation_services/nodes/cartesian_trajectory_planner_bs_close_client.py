...
def trajectory_planner_bs_open_client(grab_Y, grab_Z, pin_X, pin_Y, pin_Z, degree, points):
    ...
    try:
        cartesian_path_bs_open = rospy.ServiceProxy('cartesian_path_bs_close', # service name
...
def usage():
        return "Usage: rosrun pinhandling_manipulation_services cartesian_trajectory_planner_bs_close_client.py <grabbing coordinate Y relative to pin center> <grabbing coordinate Z relative to pin center> <pin center X relative to base_link> <pin center Y relative to base_link> <pin center Z relative to base_link> <circular degree> <number of points>"
...
if __name__ == '__main__':
    if len(sys.argv) == 8:
        ...
        points = int(sys.argv[7])
         ...
         print(usage())
         sys.exit(1)
    print("Requesting Cartesian Trajectory calculation for backside pin close")
