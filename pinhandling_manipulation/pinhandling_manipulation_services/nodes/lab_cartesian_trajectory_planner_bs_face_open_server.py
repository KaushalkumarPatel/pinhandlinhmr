...
class EETrajectoryCalculatorNode:
    ...
    def __init__(self):
        rospy.init_node("trajectory_planner_bs_face_open_server")

        rospy.Service('cartesian_path_bs_face_open',    # service name
    ...
    def calculate_trajectory(self, req):
        ...
        firsteedegree = math.radians(180.0)
        ...
        for i in range(points):
            ...
            qx = 0.0
            qy = math.sin((firsteedegree - i * Δdegree) / 2)
            qz = -math.cos((firsteedegree - i * Δdegree) / 2)
            qw = 0.0
