...
class PinCloseBSActionServer():
    ...
    def publish_position(self, goal):
        ...
        if goal:
            ...
            rospy.loginfo('\n%s: Executing, pin backside closing action server, current arm configuration:\nOrientation w= %s\nOrientation x= %s\nOrientation y= %s\nOrientation z= %s\nPosition x= %s\nPosition y= %s\nPosition z= %s'
...
if __name__ == '__main__':
    rospy.init_node('pin_close_bs_action')
    server = PinCloseBSActionServer(rospy.get_name())
    rospy.spin()