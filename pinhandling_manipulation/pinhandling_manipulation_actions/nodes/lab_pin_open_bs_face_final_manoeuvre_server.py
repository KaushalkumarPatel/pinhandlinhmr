...
class PinOpenBSFaceFinalManoeuvreActionServer():
    ...
    def publish_position(self, goal):
        ...
        if goal:
            ...
            pose_goal.orientation.x = resp1.trajectory[len(resp1.trajectory) - 7]
            pose_goal.orientation.y = resp1.trajectory[len(resp1.trajectory) - 6]
            pose_goal.orientation.z = resp1.trajectory[len(resp1.trajectory) - 5]
            pose_goal.orientation.w = resp1.trajectory[len(resp1.trajectory) - 4]
            pose_goal.position.x = resp1.trajectory[len(resp1.trajectory) - 3] - 0.2
            pose_goal.position.y = resp1.trajectory[len(resp1.trajectory) - 2]
            pose_goal.position.z = resp1.trajectory[len(resp1.trajectory) - 1]
...
if __name__ == '__main__':
    rospy.init_node('pin_open_bs_face_last_manoeuvre_action')
    server = PinOpenBSFaceFinalManoeuvreActionServer(rospy.get_name())
    rospy.spin()